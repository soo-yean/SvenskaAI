...
import { BookOpen, MessageCircle, GraduationCap, Languages, Menu, X } from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-900 text-white overflow-hidden">
      ...
      <div className="md:hidden bg-slate-800 p-4 flex justify-between items-center border-b border-slate-700 z-50">
        <div className="flex items-center space-x-2">
           ...
           <span className="text-xl font-bold tracking-tight">Svenska<span className="text-sweden-blue">Ai</span></span>
        </div>
      ...
      {/* Sidebar Navigation */}
      <div className={`
        fixed inset-0 z-40 bg-slate-900/95 backdrop-blur-sm transition-transform duration-300 md:relative md:translate-x-0 md:bg-slate-800 md:w-64 md:border-r md:border-slate-700 md:flex md:flex-col
        ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-6 hidden md:flex items-center space-x-2 mb-6">
          <div className="w-10 h-10 rounded-full bg-sweden-blue flex items-center justify-center text-sweden-yellow font-bold text-xl border-2 border-sweden-yellow shadow-[0_0_15px_rgba(254,204,0,0.3)]">
             S
          </div>
          <span className="text-2xl font-bold tracking-tight">Svenska<span className="text-sweden-blue">Ai</span></span>
        </div>

        <nav className="flex-1 px-4 space-y-2 pt-4 md:pt-0">
          <NavItem view={AppView.DASHBOARD} icon={BookOpen} label="Daglig Dos" />
          <NavItem view={AppView.CHAT} icon={MessageCircle} label="Tutor Sven" />
          <NavItem view={AppView.QUIZ} icon={GraduationCap} label="Quiz Arena" />
          <NavItem view={AppView.TRANSLATE} icon={Languages} label="Översättare" />
        </nav>
